...
// Note:
	// I have added some additional lines of code at the end of the GUI Composition section of the constructor method
	// These are to optomize the appearance of the GUI specifically for mac
	// If you encounter any strange visual issues with the GUI because it is running on a Windows Computer, please comment these out and try again
...
public class ChatRoomClient extends JFrame implements ActionListener 
{
	...
	public ChatRoomClient(String serverAddress, String clientName, String password) throws Exception  	// My constructor method
	{
	    ...
	    whosInLabel.setForeground(Color.blue);						// instructions told us to add this even through it is not in your provided .class file
	    
	    //UIManager.setLookAndFeel("javax.swing.plaf.metal.MetalLookAndFeel"); 		//Add this line to help Mac GUIs look better:
	    // Alternative Method for changing the appearance for Macs:
	    ...
	    // My additions to make everything look correct on my mac:
	    sendPublicButton.setOpaque(true);
	    sendPublicButton.setBorderPainted(false);
	    //errMsgTextField.setOpaque(true);
	    errMsgTextField.setBorder(null);
	    sendMsgLabel.setFont(new Font("default", Font.PLAIN, 12));
	    
	} // end of constructor

	
	public void receive() 	// receive() method:
	{
		try {
			while(true) 														// capture entering main thread in a do-forever read loop
				...
				if (incomingMessage instanceof String)							// if you receive a String 
		        	{
			...
			{
			errMsgTextField.setFont(new Font("default", Font.BOLD, 10));// added to fit whole message in box
			// show error message to user
		    errMsgTextField.setBackground(Color.pink); 					// change error message background to pink
	...
	public void actionPerformed(ActionEvent ae)							// buttons etc will call here
		...
		if (ae.getSource() == sendPublicButton) 						// is the sendPublicButton calling us?
		   ...
		   try {														// Send() Function?
			   ...
			   { 
			   // Take no action.
			   // If the connection to the server is down, the readObject() method in the ois has no-doubt already seen it and
			   // the catch block in receive() is telling the user they must restart the client to re-establish connection with the chat server.
			   } 
	...
	public static void main(String[] args) throws Exception 
	{
		...
		if (args.length ==3) 
			...
			try {
		    	...
		    	System.out.println("Please restart client to try again");
